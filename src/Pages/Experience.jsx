import React from "react";
import "./exp.scss";

const experiences = [
  {
    role: "Front-End Developer - Startup",
    company: "GoTrack - Logistic Platform",
    date: "June 2024",
    logo: "data:image/png;base64,", // Şirkət loqosunun fayl adını əlavə et
    details: [
      "Developed and maintained user interfaces with a focus on responsiveness and user experience.",
      "Worked collaboratively with the development team to implement new features using React and Redux.",
      "Optimized front-end performance and ensured compatibility across various browsers."
    ]
  },
  {
    role: "Front-End Developer Intern",
    company: "Javachip Digital Solution",
    date: "January 2025 - Present",
    logo: "https://thejavachip.com/static/media/favicon.46b6658e648123c737ec.png", // Şirkət loqosunun fayl adını əlavə et
    details: [
      "Assisted in building web applications by utilizing React and TypeScript.",
      "Collaborated with designers to implement visually appealing UI/UX elements.",
      "Participated in code reviews and provided feedback to improve code quality and maintainability.",
      "Engaged in learning and applying modern front-end development best practices."
    ]
  }
];

const Experience = () => {
  return (
    <div className="container">
      <div className="experience__header">Experience</div>
      <div className="card__container">
        {experiences.map((exp, index) => (
          <div className="card_inner" key={index}>
            <div className="exp__header">
              <img src={exp.logo} alt={`${exp.company} logo`} className="exp__logo" />
              <div className="exp__info">
                <h3>{exp.role}</h3>
                <h4>{exp.company}</h4>
                <p className="exp__date">{exp.date}</p>
              </div>
            </div>
            <ul className="exp__details">
              {exp.details.map((detail, i) => (
                <li key={i}>{detail}</li>
              ))}
            </ul>
          </div>
        ))}
      </div>
    </div>
  );
};

export default Experience;
